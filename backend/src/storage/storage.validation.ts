...
export function validateStorageFile(input: StorageFile): string {
  const extension = extensionFor(input.filename);
  const expectedExtension = ALLOWED_TYPES.get(input.mimeType);
  ...
  if (input.file.length > MAX_FILE_SIZE) throw new StorageValidationError("File exceeds the 10 MB limit");
